...
exports.addPosts = (req, res, next) => {
    ...
    if(!username || !password || !actions){
        ...
        database
            .execute("INSERT INTO post (post,username,password,actions) VALUES (NULL,?,?,?)",
            [
                username,
                password,
                actions.join(',')
    ...
    if(!posts){
        ...
        if(posts.length > 0){
            ...
            for(let i = 0 ; i < posts.length; i++){
                try{
                let update =await database.execute('update post set username=?,password=?,actions=? where post=?',
                [posts[i].username,posts[i].password,this.joinedActions(posts[i].actions),posts[i].post]
                )
                }catch(err){
                    if(!err.statusCode) err.statusCode =500;
                    next(err);
...
exports.addActions = (req, res, next) => {
    ...
    if(!actions || !name){
        ...
        database
            .execute("INSERT INTO actions (actions,name) VALUES (NULL,?)",
            [name]
            );
...
exports.getActions = async(action) =>{
    ...
    for (let i = 0 ; i < actions.length ; i++){
        let sActions = await database.execute('select * from actions where actions =?',[actions[i]])
